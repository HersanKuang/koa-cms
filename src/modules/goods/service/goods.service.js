...
class GoodsService {
  ...
  async getCategoryCount() {
    ...
    const queryStatement = 'SELECT COUNT(*) AS goodsCount FROM products WHERE categoryId = ?;'
    const [result] = await connection.query(statement, [statement])

    for (const item of result) {
      const [[{ goodsCount  }]] = await connection.query(queryStatement, [item.id])
  ...
  async getCategoryFavor() {
    const statement = 'SELECT id, name FROM category;'
    const queryStatement = 'SELECT categoryId, SUM(favorCount) AS goodsFavor FROM products WHERE categoryId = ?;'
    const [result] = await connection.query(statement, [statement])

    for (const item of result) {
      const [[{ goodsFavor  }]] = await connection.query(queryStatement, [item.id])
  ...
  async getAmount() {
    const keys = Object.keys(this.amountMap)
    const data = []
  ...
  async getCategorySale() {
    const statement = 'SELECT id, name FROM category;'
    const queryStatement = 'SELECT SUM(saleCount) AS goodsCount FROM products WHERE categoryId = ?;'
    const [result] = await connection.query(statement, [statement])
  ...
  async getAddressSale() {
    const statement = 'SELECT address, SUM(saleCount) AS count FROM products GROUP BY address;'
    const [result] = await connection.execute(statement)
    return result
  }
}
